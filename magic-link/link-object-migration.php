...
class Disciple_Tools_Magic_Links_Object_Migration {

    public static $migration_schema_latest_version_id = '1.0';
    public static $migration_schema_blank_version_id = 'blank';
    ...
    private function build_migration_schemas(): array {
        return [
            self::$migration_schema_blank_version_id . '->1.0' => [
                'enabled' => true,
                'schema'  => [
                    [
    ...
    private function migrate(): void {
        ...
        $migration_schemas = self::build_migration_schemas();
        ...
        foreach ( Disciple_Tools_Bulk_Magic_Link_Sender_API::fetch_option_link_objs() ?? [] as $link_obj ) {
            if ( ! empty( $link_obj ) ) {
                ...
                $link_obj_version = $link_obj['version'] ?? self::$migration_schema_blank_version_id;

                // Is link object storage shape @ the latest version?
                if ( $link_obj_version != self::$migration_schema_latest_version_id ) {

                    // Attempt to locate a suitable schema to facilitate migration.
                    $schema_key = $link_obj_version . '->' . self::$migration_schema_latest_version_id;
                    if ( isset( $migration_schemas[ $schema_key ] ) && $migration_schemas[ $schema_key ]['enabled'] ) {

                        // Prepare new link object skeleton.
                        $new_link_obj = [];

                        // First, process all direct map based schema migration rules.
                        foreach ( $migration_schemas[ $schema_key ]['schema'] ?? [] as $rule ) {
...
Disciple_Tools_Magic_Links_Object_Migration::instance();
