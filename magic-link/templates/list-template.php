...
add_action('dt_magic_link_template_load', function ( $template ) {
    if ( isset( $template['type'] ) && $template['type'] === 'list-template' ) {
        new Disciple_Tools_Magic_Links_Template_List( $template );
    }
} );
 ...
 */
class Disciple_Tools_Magic_Links_Template_List extends DT_Magic_Url_Base {

    protected $template_type = 'list-template';
    public $page_title = 'List Template';
    ...
    public $type_name = '';
    public $post_type = 'user'; // Main post type that the ML is linked to.
    public $record_post_type = 'contacts'; // Child post type determined by the connection field selected
    private $post = null;
    private $items = [];
    private $meta_key = '';
    private $user = null;
...
//        $this->post = DT_Posts::get_post( $this->post_type, $this->parts['post_id'], true, false );
        ...
        $query_fields = [];
        if ( !empty( $this->template['query'] ) ) {
            $query_fields = $this->template['query'];
        }
    ...
    public function add_endpoints() {
        ...
        register_rest_route(
            $namespace, '/' . $this->type . '/search-users', [
                [
                    'methods'             => 'POST',
    ...
    public function get_post( WP_REST_Request $request ){
        ...
        $auth_type = $params['parts']['post_type'] === 'user' ? 'user' : 'post';
        $auth_id = $params['parts']['post_id'];

        // Update logged-in user state if required accordingly, based on their sys_type
        if ( !is_user_logged_in() ) {
            DT_ML_Helper::update_user_logged_in_state( $auth_type, $auth_id );
        }
    ...
    public function search_users( WP_REST_Request $request ) {
        ...
        return Disciple_Tools_Users::get_assignable_users_compact( $search, false, $this->record_post_type );
    }
}
